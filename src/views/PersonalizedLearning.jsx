...
const PersonalizedLearning = () => {
  ...
  const fetchQuizStatsAndHistory = async () => {
    ...
    try {
      ...
      const [statsRes, historyRes] = await Promise.all([
        fetch('http://localhost:5000/api/quiz/stats', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('http://localhost:5000/api/quiz/history', { headers: { 'Authorization': `Bearer ${token}` } })
      ]);
      if (statsRes.ok) {
  ...
  const handleSubmitInitialAnswer = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5000/api/quiz/submit-answer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleSubmitDefense = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5000/api/quiz/evaluate-defense', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleNextQuestion = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://localhost:5000/api/quiz/next-question', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
